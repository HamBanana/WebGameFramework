...
1. **Sprite animators** — every Cell, Player, and PlayerStructure carries an animator instance. The framework's `SpriteSystem` ticks them via `animator.update(dt)` (called from `AccaGame._update`). Most v2 sprites are single-frame, but the die has a multi-frame "rolling" state,